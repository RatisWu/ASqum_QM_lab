...
class Parameters(NodeParameters):
    ...
    amp_factor_step: float = 0.02 #005
...
node = QualibrationNode(name="ArbWaveform_Power_Rabi", parameters=Parameters())
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        ...
        ds = ds.assign_coords(
        {
            "abs_amp": (
                ["qubit", "amp"],
                np.array([1.0 * amps for q in qubits]),
            )
            }
        )
    else:
    ...
    if N_pi == 1:
        ...
        for q in qubits:
            ...
            if factor <= 1.0: #limits.max_x180_wf_amplitude:
                print(f"amplitude for Pi pulse is modified by a factor of {factor:.2f}")
                print(f"new amplitude factor is {factor}\n")
                fit_results[q.name]["amplitude_factor"] = factor
            else:
                print(f"Fitted amplitude too high, factor = {factor}\n")
                fit_results[q.name]["amplitude_factor"] = 1
    ...
    for ax, qubit in grid_iter(grid):
        if N_pi == 1:
            if state_discrimination:
                ds.assign_coords(amp_mV=ds.abs_amp).loc[qubit].state.plot(ax=ax, x="amp_mV")
                ax.plot(ds.abs_amp.loc[qubit], fit_evals.loc[qubit][0])
                ax.set_ylabel("Qubit state")
            else:
                (ds.assign_coords(amp_mV=ds.abs_amp).loc[qubit].I * 1e3).plot(ax=ax, x="amp_mV")
    ...
    if node.parameters.load_data_id is None:
        with node.record_state_updates():
            for q in qubits:
                if hasattr(q.xy.operations[operation], "sn"):
                    I_samples = fit_results[q.name]["amplitude_factor"]*np.array(q.xy.operations[operation].I_samples)
                    q.xy.operations[operation].I_samples = I_samples.tolist()
                    Q_samples = fit_results[q.name]["amplitude_factor"]*np.array(q.xy.operations[operation].Q_samples)
                    q.xy.operations[operation].Q_samples = Q_samples.tolist()
                    if operation == "x180" and node.parameters.update_x90:
                        if hasattr(q.xy.operations['x90'], "sn"):
                            I_samples = fit_results[q.name]["amplitude_factor"]*np.array(q.xy.operations['x90'].I_samples)
                            q.xy.operations['x90'].I_samples = I_samples.tolist()
                            Q_samples = fit_results[q.name]["amplitude_factor"]*np.array(q.xy.operations['x90'].Q_samples)
                            q.xy.operations['x90'].Q_samples = Q_samples.tolist()
                        else:
                            raise AttributeError("The operation 'x90' doesn't have the attribute 'sn' skip update its samples !")
                else:
                    raise AttributeError(f"Interesting ! Your operation {operation} doesn't have the attribute 'sn' but you still can run this node...")
